...
trait DecoderInstances:
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] = {
    def flattenOptionList[T](list: List[Option[T]], result: List[T]): List[T] = list match
      case Nil => result.reverse
      case Some(x) :: rest => flattenOptionList(rest, x :: result)
      case _ => Nil

    Decoder.fromPartialFunction {
      case Json.Arr(as) => flattenOptionList(as.map(decoder.decode), List())
    }
  }
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction {
      case Json.Obj(map) => map.get(name).flatMap(decoder.decode)
      case _ => None
    }
