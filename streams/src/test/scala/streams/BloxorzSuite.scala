...
class BloxorzSuite extends munit.FunSuite:
  ...
  trait Level1 extends SolutionChecker:
    ...
    val history = List(Move.Left, Move.Up)

    val exploredPos = Set(Block(Pos(1, 2), Pos(1, 3)), Block(Pos(1, 1), Pos(1, 1)))
  ...
  test("get neighbors with history") {
    new Level1:
      assertEquals(neighborsWithHistory(block, history), neighbors.to(LazyList))
  }

  test("ignore already explored positions") {
    new Level1:
      assertEquals(newNeighborsOnly(neighbors.to(LazyList), exploredPos), newNeighbors)
  }
